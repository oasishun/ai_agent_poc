...
class OrchestratorState(TypedDict):
    ...
    session_id: str
    conversation_history: list  # 대화 문맥 유지를 위한 이전 메시지 기록

# Graph 구성
graph = StateGraph(OrchestratorState)
graph.add_node("router", route_intent)         # 의도 분석 (과거 문맥 포함) → Agent 라우팅
graph.add_node("booking", booking_agent_node)   # Booking Agent 실행
graph.add_node("tracking", tracking_agent_node) # Tracking Agent 실행
graph.add_node("general", general_node)         # 일반 문의 응답
graph.add_edge("router", "booking")             # conditional edge (continue 의도 포함)
...
#### Conversational Memory Routing
사용자가 짧은 문장("옵션 2로 해줘", "그렇게 해줘")으로 답변할 때 맥락이 끊기는 것을 방지하기 위해:
1. 라우터 평가 시 최근 대화 내용(Conversation History)을 함께 주입
2. 현재 진행 중인 대화의 연장선이라고 판단될 경우 `continue` 상태를 부여하여 활성화된 Agent의 컨텍스트를 유지하도록 라우팅
...
| Level | Color | Content |
|---|---|---|
| `DEBUG` | Gray | LLM prompt/response 전문, Agent 내부 추론 과정(`Agent reasoning/response`), embedding 벡터, raw API |
| `INFO` | Cyan | Agent 전환, Tool 호출/결과(`Agent decided to execute`), Knowledge 검색 완료, Decision Trace 생성 |
...
- Agent가 의사결정을 위해 판단(Thinking)하는 시간 동안 "Agent is thinking..." 등의 진행 상황 로그 표출
